...
int main(int argc, char * argv[])
{
  ...
  if( argc < 4 )
    {
    std::cerr << "usage: " << argv[0] << " intput kernel output [alpha [method [gpf]]]" << std::endl;
  ...
  typedef itk::MaximumAPosterioriLeastSquaresDeconvolutionImageFilter< IType > FFTConvolutionType;
  ...
  // test default value
  TEST_SET_GET_VALUE( 0.001f, conv->GetAlpha() );
  ...
  if( argc >= 5 )
    {
    conv->SetAlpha( atof(argv[4]) );
    // TEST_SET_GET_VALUE( atof(argv[4]), conv->GetAlpha() );
    }
  if( argc >= 6 )
    {
