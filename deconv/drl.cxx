...
int main(int argc, char * argv[])
{
  ...
  if( argc < 4 )
    {
    std::cerr << "usage: " << argv[0] << " intput kernel output [threshold [nbIt [th [method [gpf]]]]]" << std::endl;
  ...
  typedef itk::DampedRichardsonLucyDeconvolutionImageFilter< IType > FFTConvolutionType;
  ...
  if( argc >= 5 )
    {
    conv->SetThreshold( atof(argv[4]) );
    // TEST_SET_GET_VALUE( atof(argv[4]), conv->GetThreshold() );
    }
